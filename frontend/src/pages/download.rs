#[path = "../components/file.rs"] mod file;
#[path = "../components/file_not_found.rs"] mod file_not_found;
...
fn overview(p: &DownloadProps) -> HtmlResult {
    ...
    let fetch = use_future(|| async {
        let result: Result<FileMeta, gloo_net::Error> = Request::new("api/meta").query(query).send().await?.json().await;
        result
    })?;
